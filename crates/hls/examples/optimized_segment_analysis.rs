...
fn main() {
    ...
    println!("  Average per segment: {:?}", duration / segments.len() as u32);
    println!("  Segments per second: {:.1}", segments.len() as f64 / duration.as_secs_f64());
}
...
fn analyze_segments(segments: &[HlsData]) {
    for (i, segment) in segments.iter().enumerate() {
        ...
        println!("  Type: {:?} ({} bytes)", segment.segment_type(), segment.size());
...
fn analyze_ts_segment(segment: &HlsData, segment_num: usize) {
    ...
    println!("    Has video: {}", if segment.has_video_streams() { "✓" } else { "✗" });
    println!("    Has audio: {}", if segment.has_audio_streams() { "✓" } else { "✗" });
    ...
    if let Some(profile) = segment.get_stream_profile() {
        ...
        println!("    Complete stream: {}", if profile.is_complete() { "✓" } else { "✗" });
...
fn show_detailed_streams(segment: &HlsData) {
    ...
    if let Some(Ok(streams)) = segment.get_ts_all_streams() {
        for (pid, stream_type) in streams {
            let category = if stream_type.is_video() { "Video" } else if stream_type.is_audio() { "Audio" } else { "Other" };
            println!("    PID 0x{:04X}: {} ({:?})", pid, category, stream_type);
        }
    }
...
fn create_test_segments() -> Vec<HlsData> {
    ...
    // Segment 1: H.264 + AAC
    segments.push(create_h264_aac_segment());
    
    // Segment 2: H.265 + AC-3
    segments.push(create_h265_ac3_segment());
    
    // Segment 3: Multiple programs
    segments.push(create_multi_program_segment());
    
    // Segment 4: End marker
    segments.push(HlsData::end_marker());
...
fn create_ts_data_h264_aac() -> Vec<u8> {
    ...
    pat_packet[8] = 0x00; pat_packet[9] = 0x01; // Transport stream ID
    pat_packet[10] = 0x01; // Version + current/next
    pat_packet[11] = 0x00; pat_packet[12] = 0x00; // Section numbers
    // Program entry
    ...
    pmt_packet[8] = 0x00; pmt_packet[9] = 0x01; // Program number 1
    pmt_packet[10] = 0x01; // Version + current/next
    pmt_packet[11] = 0x00; pmt_packet[12] = 0x00; // Section numbers
    pmt_packet[13] = 0xE1; pmt_packet[14] = 0x00; // PCR PID 0x100
    pmt_packet[15] = 0x00; pmt_packet[16] = 0x00; // Program info length
    // Video stream (H.264)
    pmt_packet[17] = 0x1B; // H.264
    pmt_packet[18] = 0xE1; pmt_packet[19] = 0x00; // PID 0x100
    pmt_packet[20] = 0x00; pmt_packet[21] = 0x00; // ES info length
    // Audio stream (AAC)
    pmt_packet[22] = 0x0F; // ADTS AAC
    pmt_packet[23] = 0xE1; pmt_packet[24] = 0x01; // PID 0x101
    pmt_packet[25] = 0x00; pmt_packet[26] = 0x00; // ES info length
...
fn create_ts_data_h265_ac3() -> Vec<u8> {
    ...
    let mut pat_packet = vec![0u8; 188];
    pat_packet[0] = 0x47;
    pat_packet[1] = 0x40; pat_packet[2] = 0x00; pat_packet[3] = 0x10;
    pat_packet[4] = 0x00; pat_packet[5] = 0x00; pat_packet[6] = 0x80; pat_packet[7] = 0x0D;
    pat_packet[8] = 0x00; pat_packet[9] = 0x01; pat_packet[10] = 0x01;
    pat_packet[11] = 0x00; pat_packet[12] = 0x00;
    pat_packet[13] = 0x00; pat_packet[14] = 0x01;
    pat_packet[15] = 0xE1; pat_packet[16] = 0x00;

    // PMT packet (H.265 + AC-3)
    let mut pmt_packet = vec![0u8; 188];
    pmt_packet[0] = 0x47;
    pmt_packet[1] = 0x41; pmt_packet[2] = 0x00; pmt_packet[3] = 0x10;
    pmt_packet[4] = 0x00; pmt_packet[5] = 0x02; pmt_packet[6] = 0x80; pmt_packet[7] = 0x17;
    pmt_packet[8] = 0x00; pmt_packet[9] = 0x01; pmt_packet[10] = 0x01;
    pmt_packet[11] = 0x00; pmt_packet[12] = 0x00;
    pmt_packet[13] = 0xE1; pmt_packet[14] = 0x00;
    pmt_packet[15] = 0x00; pmt_packet[16] = 0x00;
    // Video stream (H.265)
    pmt_packet[17] = 0x24; // H.265
    pmt_packet[18] = 0xE1; pmt_packet[19] = 0x00; // PID 0x100
    pmt_packet[20] = 0x00; pmt_packet[21] = 0x00;
    // Audio stream (AC-3)
    pmt_packet[22] = 0x81; // AC-3
    pmt_packet[23] = 0xE1; pmt_packet[24] = 0x01; // PID 0x101
    pmt_packet[25] = 0x00; pmt_packet[26] = 0x00;
...
fn create_ts_data_multi_program() -> Vec<u8> {
    ...
    pat_packet[0] = 0x47;
    pat_packet[1] = 0x40; pat_packet[2] = 0x00; pat_packet[3] = 0x10;
    pat_packet[4] = 0x00; pat_packet[5] = 0x00; pat_packet[6] = 0x80; pat_packet[7] = 0x11;
    pat_packet[8] = 0x00; pat_packet[9] = 0x01; pat_packet[10] = 0x01;
    pat_packet[11] = 0x00; pat_packet[12] = 0x00;
    // Program 1
    pat_packet[13] = 0x00; pat_packet[14] = 0x01;
    pat_packet[15] = 0xE1; pat_packet[16] = 0x00;
    // Program 2
    pat_packet[17] = 0x00; pat_packet[18] = 0x02;
    pat_packet[19] = 0xE2; pat_packet[20] = 0x00;

    // PMT 1 (H.264 + AAC)
    let mut pmt1_packet = vec![0u8; 188];
    pmt1_packet[0] = 0x47;
    pmt1_packet[1] = 0x41; pmt1_packet[2] = 0x00; pmt1_packet[3] = 0x10;
    pmt1_packet[4] = 0x00; pmt1_packet[5] = 0x02; pmt1_packet[6] = 0x80; pmt1_packet[7] = 0x17;
    pmt1_packet[8] = 0x00; pmt1_packet[9] = 0x01;
    pmt1_packet[10] = 0x01; pmt1_packet[11] = 0x00; pmt1_packet[12] = 0x00;
    pmt1_packet[13] = 0xE1; pmt1_packet[14] = 0x00;
    pmt1_packet[15] = 0x00; pmt1_packet[16] = 0x00;
    pmt1_packet[17] = 0x1B; // H.264
    pmt1_packet[18] = 0xE1; pmt1_packet[19] = 0x00;
    pmt1_packet[20] = 0x00; pmt1_packet[21] = 0x00;
    pmt1_packet[22] = 0x0F; // AAC
    pmt1_packet[23] = 0xE1; pmt1_packet[24] = 0x01;
    pmt1_packet[25] = 0x00; pmt1_packet[26] = 0x00;

    // PMT 2 (H.265 + AC-3)
    let mut pmt2_packet = vec![0u8; 188];
    pmt2_packet[0] = 0x47;
    pmt2_packet[1] = 0x42; pmt2_packet[2] = 0x00; pmt2_packet[3] = 0x10;
    pmt2_packet[4] = 0x00; pmt2_packet[5] = 0x02; pmt2_packet[6] = 0x80; pmt2_packet[7] = 0x17;
    pmt2_packet[8] = 0x00; pmt2_packet[9] = 0x02;
    pmt2_packet[10] = 0x01; pmt2_packet[11] = 0x00; pmt2_packet[12] = 0x00;
    pmt2_packet[13] = 0xE2; pmt2_packet[14] = 0x00;
    pmt2_packet[15] = 0x00; pmt2_packet[16] = 0x00;
    pmt2_packet[17] = 0x24; // H.265
    pmt2_packet[18] = 0xE2; pmt2_packet[19] = 0x00;
    pmt2_packet[20] = 0x00; pmt2_packet[21] = 0x00;
    pmt2_packet[22] = 0x81; // AC-3
    pmt2_packet[23] = 0xE2; pmt2_packet[24] = 0x01;
    pmt2_packet[25] = 0x00; pmt2_packet[26] = 0x00;
