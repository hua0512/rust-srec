use hls::{HlsData, TsStreamInfo, ProgramInfo};
use ts::StreamType;
use bytes::Bytes;
...
fn compare_parsing_approaches(hls_data: &HlsData) {
    ...
    match traditional_result {
        Some(Ok((pat, pmts))) => {
            ...
            if let Some(pat) = pat {
                ...
                println!("  Estimated memory: ~{} bytes", memory_estimate);
            }
    ...
    match zero_copy_result {
        Some(Ok(stream_info)) => {
            ...
            println!("  Actual memory: ~{} bytes", memory_usage);
            
            for program in &stream_info.programs {
                println!("    Program {}: {} video, {} audio, {} other streams",
    ...
    if traditional_duration > zero_copy_duration {
        ...
        let slowdown = zero_copy_duration.as_nanos() as f64 / traditional_duration.as_nanos() as f64;
        println!("⚡ Zero-copy is {:.2}x slower (but uses less memory)", slowdown);
    }
}
