...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      document.cookie = `${SIDEBAR_COOKIE_KEY}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE_SECONDS}`;
    },
    [setOpenProp, open],
  );
  ...
  if (collapsible === 'none') {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          'bg-sidebar/80 backdrop-blur-xl text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col border-border/50',
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'relative w-(--sidebar-width) bg-transparent transition-[width] duration-(--sidebar-duration) ease-(--sidebar-ease)',
      ...
      <div
        ...
        className={cn(
          'fixed inset-y-0 z-20 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-(--sidebar-duration) ease-(--sidebar-ease) md:flex',
          side === 'left'
          ...
          variant === 'floating' || variant === 'inset'
            ...
            : 'group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=left]:border-r group-data-[side=right]:border-l border-border/50',
          className,
        )}
        {...props}
      >
