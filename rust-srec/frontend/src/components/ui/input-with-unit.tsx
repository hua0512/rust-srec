...
interface InputWithUnitProps extends Omit<React.ComponentProps<"div">, "onChange"> {
...
export function InputWithUnit({
    ...
    React.useEffect(() => {
        ...
        if (lastEmittedValue.current !== undefined && value === lastEmittedValue.current) {
            return;
        }
    ...
    return (
        ...
        >
            ...
            >
                <SelectTrigger
                    className="w-[85px] border-0 shadow-none focus:ring-0 focus:ring-offset-0 rounded-l-none h-9 px-3 gap-1 hover:bg-muted/50"
                >
