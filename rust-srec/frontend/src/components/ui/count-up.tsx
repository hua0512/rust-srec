...
export function CountUp({ value, formatter, className, duration = 0.8 }: CountUpProps) {
    ...
    useEffect(() => {
        ...
        const controls = animate(start, target, {
            ...
            onUpdate: (latest) => {
                if (nodeRef.current) {
                    nodeRef.current.textContent = formatter ? formatter(latest) : Math.round(latest).toString();
                }
            }
        });
