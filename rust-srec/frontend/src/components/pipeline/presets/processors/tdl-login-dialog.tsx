...
function extractFirstUrl(text: string): string | null {
    const match = text.match(/(tg:\/\/[^\s)]+|https?:\/\/[^\s)]+|t\.me\/[^\s)]+)/i);
    const raw = match?.[0];
    if (!raw) return null;
            ...
            <DialogContent className="sm:max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl">
                            ...
                            <div className="mb-3 space-y-2">
                                {isWindowsError && (
                                    <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-3">
                                        ...
                                        <div className="text-[11px] text-muted-foreground mt-1">
                                            <Trans>
                                                Interactive password entry via this API is not supported on
                                                Windows. Please use "Desktop Login" (import tdata) or
                                ...
                                {isTdlNotFound && (
                                    <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-3">
                                        ...
                                        <div className="text-[11px] text-muted-foreground mt-1">
                                            <Trans>
                                                Set "TDL Binary Path" to your `tdl` executable, or make sure `tdl` is in PATH.
                                            </Trans>
                                        </div>
                                    </div>
                                )}
                                {isPasswordBlocked && !allowPassword && (
                                    <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-3">
                                        ...
                                        <div className="text-[11px] text-muted-foreground mt-1">
                                            <Trans>
                                                This login requires a Telegram 2FA password. Enable "Allow 2FA" and restart.
                                            </Trans>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-3">
                                        ...
                                        <div className="mt-2 flex items-center gap-2">
                                            <Input
                                                ...
                                                placeholder={t(i18n)`Path to Telegram Desktop (contains tdata)`}
                                            ...
                                            <Button
                                                ...
                                                onClick={() => {
                                                    if (!desktopDir.trim()) {
                                                        toast.error(
                                                            t(i18n)`Please set Telegram Desktop Directory first`,
                                                        );
                                                        return;
                                                    }
                                                    if (sessionId) {
                ...
                <DialogFooter className="p-6 pt-2 bg-muted/30 border-t gap-3 sm:gap-0">
                    ...
                    >
                        ...
                        <Button
                            type="submit"
                            disabled={!isRunning || !inputTask.trim() || sendMutation.isPending}
