...
export const DEFAULT_JOB_PRESET_NAMES: Record<string, any> = {
  ...
  'preset-default-upload-baidupcs': msg`Upload (Baidu Netdisk)`,
  ...
  'preset-default-execute': msg`Execute Command`,
...
export const DEFAULT_JOB_PRESET_DESCRIPTIONS: Record<string, any> = {
  ...
  'preset-default-upload-baidupcs': msg`Upload files to Baidu Netdisk using BaiduPCS-Go. Configure the account and destination before use.`,
  ...
  'preset-default-execute': msg`Run a custom shell command with placeholders (e.g. {input}, {inputs_json}, {streamer}, %Y%m%d).`,
  ...
  'preset-default-danmu-to-ass': msg`Convert danmu XML (Bilibili-style) into .ass subtitles using DanmakuFactory. Pairs per segment and is batch-safe.`,
  'preset-default-ass-burnin': msg`Burn .ass subtitles into videos (produces *_burnin.mp4 by default). Pairs per segment and is batch-safe.`,
