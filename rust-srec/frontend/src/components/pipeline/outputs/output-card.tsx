...
export function OutputCard({ output }: OutputCardProps) {
    ...
    const colorClass = FORMAT_COLORS[formatLower] || "from-gray-500/10 to-gray-500/5 text-gray-500 border-gray-500/20";
    ...
    return (
        <Card className="relative h-full flex flex-col transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/10 group overflow-hidden bg-gradient-to-br from-background/80 to-background/40 backdrop-blur-xl border-border/40 hover:border-primary/20">
            ...
            <CardHeader className="relative flex flex-row items-center gap-4 pb-2 space-y-0 z-10">
                <div className={`p-3 rounded-2xl bg-gradient-to-br ${colorClass} ring-1 ring-inset ring-black/5 dark:ring-white/5 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`}>
                    <FileVideo className="h-5 w-5" />
                </div>
                <div className="flex-1 min-w-0 space-y-1">
                    <CardTitle className="text-base font-medium truncate tracking-tight text-foreground/90 group-hover:text-primary transition-colors duration-300" title={filename}>
                        {filename}
                    </CardTitle>
                ...
                <Badge className={`bg-gradient-to-br ${colorClass} border font-mono text-xs uppercase`}>
                    {output.format}
                </Badge>
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                        ...
                        <DropdownMenuItem onClick={() => {
                            const dir = output.file_path.substring(0, output.file_path.lastIndexOf(/[\\/]/.test(output.file_path) ? (output.file_path.includes('\\') ? '\\' : '/') : '/'));
                            navigator.clipboard.writeText(dir);
                            toast.success('Directory path copied');
                        }}>
                            <FolderOpen className="mr-2 h-4 w-4" /> <Trans>Copy Directory</Trans>
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            </CardHeader>
            ...
            <CardContent className="relative pb-4 flex-1 z-10">
                <p className="text-xs text-muted-foreground/80 line-clamp-2 mb-4 leading-relaxed font-mono bg-muted/30 p-2 rounded-md" title={output.file_path}>
                    {output.file_path}
                </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center gap-2 p-2 rounded-md bg-muted/30 border border-transparent group-hover:border-primary/5 transition-colors">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[9px] uppercase tracking-wider text-muted-foreground/50">Size</span>
                            <span className="text-xs font-medium">{formatBytes(output.file_size_bytes)}</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 p-2 rounded-md bg-muted/30 border border-transparent group-hover:border-primary/5 transition-colors">
                        <Calendar className="h-3.5 w-3.5 text-muted-foreground" />
                        <div className="flex flex-col">
                            <span className="text-[9px] uppercase tracking-wider text-muted-foreground/50">Created</span>
                            <span className="text-xs font-medium">{format(new Date(output.created_at), 'PP')}</span>
                        </div>
                    </div>
                </div>
            </CardContent>
            ...
            <CardFooter className="relative pt-0 text-[10px] text-muted-foreground flex justify-between items-center z-10 border-t border-border/20 mt-auto px-6 py-3 bg-muted/5">
                <span className="font-mono opacity-50">Session: {output.session_id.substring(0, 8)}</span>
                <span className="font-mono opacity-50">{formatBytes(output.file_size_bytes)}</span>
            </CardFooter>
        </Card>
    );
}
