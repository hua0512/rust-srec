...
export function JobCard({ jobs, onRetry, onCancel, onViewDetails, onViewJob }: JobCardProps) {
    ...
    const isProcessing = sortedJobs.some(j => j.status === JobStatus.Processing);
    ...
    const overallStatus = isFailed ? JobStatus.Failed : isProcessing ? JobStatus.Processing : isCompleted ? JobStatus.Completed : isPending ? JobStatus.Pending : JobStatus.Processing;
    ...
    const processingJob = sortedJobs.find(j => j.status === JobStatus.Processing);
    ...
    return (
        ...
        >
            ...
            <CardHeader className="relative flex flex-row items-center gap-4 pb-2 space-y-0 z-10">
                <div className={`p-3 rounded-2xl ${statusConfig.color} ring-1 ring-inset ring-black/5 dark:ring-white/5 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`}>
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={(e) => {
                            ...
                            <ExternalLink className="mr-2 h-4 w-4" /> <Trans>View Details</Trans>
                        </DropdownMenuItem>
                        {isFailed && (
                            <>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={(e) => {
                                    ...
                                    <RotateCcw className="mr-2 h-4 w-4" /> <Trans>Retry Failed</Trans>
                                </DropdownMenuItem>
                            </>
                        )}
                        {(isPending || isProcessing) && (
                            <>
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            <AlertDialogTitle><Trans>Cancel Pipeline?</Trans></AlertDialogTitle>
                                            <AlertDialogDescription>
                                                <Trans>
                                                    This will cancel all pending and processing jobs in this pipeline.
                                                </Trans>
                                            </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
                                            <AlertDialogCancel><Trans>Keep Running</Trans></AlertDialogCancel>
                                            <AlertDialogAction
                                                onClick={() => {
                                                    // Stop propagation if needed, though this is in a modal
                                                    sortedJobs.forEach(job => {
                                                        const cancelableStatuses: string[] = [JobStatus.Pending, JobStatus.Processing];
            ...
            <CardContent className="relative pb-4 flex-1 z-10">
                <p className="text-xs text-muted-foreground/80 mb-4 leading-relaxed font-light">
                    <Trans>Started</Trans> {formatDistanceToNow(new Date(primaryJob.created_at), { addSuffix: true })}
                    {primaryJob.session_id && ` - Session: ${primaryJob.session_id.substring(0, 8)}`}
                </p>
                ...
                <div className="flex items-center gap-1 flex-wrap">
                    {sortedJobs.map((job, index) => {
                        ...
                        const jobStatusConfig = STATUS_CONFIG[job.status] || STATUS_CONFIG[JobStatus.Pending];
                        ...
                        return (
                            <div key={job.id} className="flex items-center">
                                ...
                                >
                                    ...
                                    {/* Status indicator */}
                                    <div className={`absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full ${jobStatusConfig.color} border shadow-sm`}>
                                        <JobStatusIcon className={`h-2.5 w-2.5 ${jobStatusConfig.animate ? 'animate-spin' : ''}`} />
                                    </div>
                                </div>
                ...
                {isCompleted && totalDuration > 0 && (
                    <div className="flex items-center gap-2 mt-3 text-xs text-muted-foreground">
                        ...
                        <span><Trans>Total time:</Trans> {formatDuration(totalDuration)}</span>
                    </div>
                )}
                ...
                {isFailed && (
                    <div className="mt-3 p-2 rounded-md bg-red-500/10 border border-red-500/20">
                        <p className="text-[10px] text-red-500 line-clamp-2">
                            {sortedJobs.find(j => j.status === JobStatus.Failed)?.error_message || <Trans>Processing failed</Trans>}
                        </p>
                    </div>
                )}
            </CardContent>
            ...
            <CardFooter className="relative pt-0 text-[10px] text-muted-foreground flex justify-between items-center z-10 border-t border-border/20 mt-auto px-6 py-3 bg-muted/5">
                ...
                {isCompleted && totalDuration > 0 && (
                    <span className="font-mono opacity-50">{formatDuration(totalDuration)}</span>
                )}
            </CardFooter>
        </Card>
    );
