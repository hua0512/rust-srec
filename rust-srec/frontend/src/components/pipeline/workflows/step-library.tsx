...
import { Search, Tag, ArrowRight, LayoutGrid, CheckCircle2, Plus, Loader2 } from 'lucide-react';
...
export function StepLibrary({ onAddStep, currentSteps }: StepLibraryProps) {
    ...
    const {
        ...
        queryFn: ({ pageParam = 0 }) => listJobPresets({
            data: {
                    ...
                    <div className="p-6 space-y-4">
                        <div className="flex items-center justify-between pb-2 border-b border-border/40">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-semibold tracking-tight text-foreground"><Trans>Step Library</Trans></h3>
                            </div>
                        ...
                        <div className="text-sm text-muted-foreground leading-relaxed">
                            <Trans>Explore matching automation steps to enhance your pipeline workflow.</Trans>
                        </div>
                        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground/70 pt-1">
                            <Tag className="h-3 w-3" />
                            <span>{data?.pages[0]?.total ?? 0} <Trans>Presets Available</Trans></span>
                        </div>
                    </div>
                </div>
            </SheetTrigger>
            <SheetContent side="left" className="w-full sm:max-w-[600px] flex flex-col p-0 gap-0 border-r border-border/60 bg-background/95 backdrop-blur-2xl shadow-2xl">
                <SheetHeader className="px-6 py-6 border-b border-border/40 space-y-4 bg-muted/5">
                    <div className="space-y-1">
                        <SheetTitle className="text-2xl font-bold tracking-tight"><Trans>Add Pipeline Step</Trans></SheetTitle>
                        <SheetDescription className="text-base">
                            <Trans>Select a processing job to step into your workflow sequence.</Trans>
                        </SheetDescription>
                    </div>
                    ...
                    <div className="p-6">
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center py-20 space-y-4">
                                ...
                                <p className="text-sm text-muted-foreground"><Trans>Loading presets...</Trans></p>
                            </div>
                        ) : (
                            <>
                                <motion.div
                                            ...
                                            return (
                                                <motion.button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isAdded && "ring-1 ring-green-500/30 bg-green-50/50 dark:bg-green-950/10"
                                                    )}
                                                >
                                                    <div className="flex items-start justify-between w-full relative z-10">
                                                        <div className={cn(
                                                            ...
                                                            isAdded ? "border-green-200 dark:border-green-800 text-green-600 dark:text-green-400" : "border-border/50 text-foreground/80"
                                                        )}>
                                                            <Icon className="h-5 w-5" />
                                                        </div>
                                                        <div className="flex items-center gap-2">
                                                            {preset.category && (
                                                                <Badge variant="secondary" className="text-[10px] h-5 px-2 font-medium opacity-70 group-hover:opacity-100 transition-opacity bg-muted/50">
                                                                    {preset.category}
                                                                </Badge>
                                                            )}
                                                            {isAdded && (
                                ...
                                {presets.length === 0 && !isLoading && (
                                    <div className="flex flex-col items-center justify-center py-20 text-center space-y-3 opacity-60">
                                        ...
                                        <div className="space-y-1">
                                            <p className="font-medium text-foreground"><Trans>No presets found</Trans></p>
                                            <p className="text-sm text-muted-foreground"><Trans>Try adjusting your search or filters</Trans></p>
                                        </div>
                                    </div>
                                )}
