...
export function DanmuConfigForm({ form, name }: DanmuConfigFormProps) {
    ...
    const updateConfig = (newConfig: DanmuSamplingConfig) => {
        setConfig(newConfig);
        form.setValue(name, JSON.stringify(newConfig), { shouldDirty: true, shouldTouch: true, shouldValidate: true });
    };
    ...
    return (
        <Card className="border-muted/40 shadow-none bg-muted/20">
            <CardContent className="p-4 space-y-4">
                <FormItem>
                    <FormLabel><Trans>Sampling Strategy</Trans></FormLabel>
                    <Select
                        onValueChange={(val) => handleStrategyChange(val as SamplingStrategy)}
                        value={config.type}
                    >
                        <FormControl>
                        ...
                        <SelectContent>
                            <SelectItem value="fixed"><Trans>Fixed Interval</Trans></SelectItem>
                            <SelectItem value="velocity"><Trans>Velocity Based (Dynamic)</Trans></SelectItem>
                        </SelectContent>
                    </Select>
                    <FormDescription className="text-xs">
                        {config.type === 'fixed'
                            ? <Trans>Sample danmu at a constant time interval.</Trans>
                            : <Trans>Adjust sampling rate based on danmu traffic volume.</Trans>
                        }
                    </FormDescription>
                </FormItem>
                ...
                {config.type === 'fixed' && (
                    <FormItem>
                        <FormLabel><Trans>Interval (seconds)</Trans></FormLabel>
                        <FormControl>
                            <Input
                                type="number"
                                min={1}
                                value={config.interval_secs}
                                onChange={(e) => updateField('interval_secs', parseInt(e.target.value) || 10)}
                ...
                {config.type === 'velocity' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormItem>
                            <FormLabel><Trans>Min Interval (s)</Trans></FormLabel>
                            <FormControl>
                                <Input
                                    type="number"
                                    min={1}
                                    value={config.min_interval_secs}
                                    onChange={(e) => updateField('min_interval_secs', parseInt(e.target.value) || 1)}
                                    className="bg-background/80"
                                />
                            </FormControl>
                        </FormItem>
                        ...
                        <FormItem>
                            <FormLabel><Trans>Max Interval (s)</Trans></FormLabel>
                            <FormControl>
                                <Input
                                    type="number"
                                    min={1}
                                    value={config.max_interval_secs}
                                    onChange={(e) => updateField('max_interval_secs', parseInt(e.target.value) || 60)}
                                    className="bg-background/80"
                                />
                            </FormControl>
                        </FormItem>
                        ...
                        <FormItem className="md:col-span-2">
                            <FormLabel><Trans>Target Danmus per Sample</Trans></FormLabel>
                            <FormControl>
                                <Input
                                    type="number"
                                    min={1}
                                    value={config.target_danmus_per_sample}
                                    onChange={(e) => updateField('target_danmus_per_sample', parseInt(e.target.value) || 100)}
                                    className="bg-background/80"
                                />
                            </FormControl>
                            <FormDescription className="text-xs">
                                <Trans>Target number of messages to capture per sample period.</Trans>
                            </FormDescription>
                        </FormItem>
                    </div>
                )}
