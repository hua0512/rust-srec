...
export function RetryPolicyForm({ form, name }: RetryPolicyFormProps) {
    ...
    const updateField = <K extends keyof RetryPolicy>(key: K, value: RetryPolicy[K]) => {
        const newPolicy = { ...policy, [key]: value };
        setPolicy(newPolicy);
        form.setValue(name, JSON.stringify(newPolicy), { shouldDirty: true, shouldTouch: true, shouldValidate: true });
    };
        ...
        <Card className="border-muted/40 shadow-none bg-muted/20">
            <CardContent className="p-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormItem>
                        <FormLabel><Trans>Max Retries</Trans></FormLabel>
                        <FormControl>
                            <Input
                                type="number"
                                min={0}
                    ...
                    <FormItem>
                        <FormLabel><Trans>Backoff Multiplier</Trans></FormLabel>
                        <FormControl>
                            <Input
                                type="number"
                                step={0.1}
                                ...
                                onChange={(e) => updateField('backoff_multiplier', parseFloat(e.target.value) || 1)}
                                className="bg-background/80"
                            />
                        </FormControl>
                        <FormDescription className="text-xs">
                    ...
                    <FormItem>
                        <FormLabel><Trans>Initial Delay (ms)</Trans></FormLabel>
                        <FormControl>
                            <Input
                                type="number"
                                min={0}
                                value={policy.initial_delay_ms}
                                onChange={(e) => updateField('initial_delay_ms', parseInt(e.target.value) || 0)}
                                className="bg-background/80"
                            />
                        </FormControl>
                    </FormItem>
                    ...
                    <FormItem>
                        <FormLabel><Trans>Max Delay (ms)</Trans></FormLabel>
                        <FormControl>
                            <Input
                                type="number"
                                min={0}
                ...
                <div className="flex flex-row items-center justify-between rounded-lg border p-3 bg-background/50">
                    <div className="space-y-0.5">
                        <FormLabel className="text-base"><Trans>Jitter</Trans></FormLabel>
                        <FormDescription className="text-xs">
                            <Trans>Add randomness to retry delays to prevent thundering herd.</Trans>
                        </FormDescription>
                    </div>
                    <Switch
