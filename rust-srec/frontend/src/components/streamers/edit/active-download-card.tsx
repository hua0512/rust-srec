...
// Reads the downloads itself so that progress ticks re-render this card
// rather than the editor page around it.
export const ActiveDownloadCard = memo(function ActiveDownloadCard({
  streamerId,
}: ActiveDownloadCardProps) {
  const downloads = useDownloadStore(
    useShallow((state) => state.getDownloadsByStreamer(streamerId)),
  );
  return (
            ...
            {download.downloadUrl && (
              <div className="col-span-2 space-y-1 overflow-hidden">
                <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                  <Link className="w-3 h-3" /> <Trans>Stream URL</Trans>
                </div>
                <Tooltip>
            ...
            {(() => {
              ...
              return (
                <div className="space-y-1 overflow-hidden">
                  <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                    <Globe className="w-3 h-3" /> <Trans>CDN</Trans>
                  </div>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
