...
export function StreamerHeader({ streamer, isRecording, isLive, platform }: StreamerHeaderProps) {
    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start gap-3 md:gap-4 overflow-hidden">
                <Button variant="ghost" size="icon" className="h-10 w-10 shrink-0 rounded-full bg-background border shadow-sm hover:bg-muted" asChild>
                    <Link to="/streamers">
                ...
                <div className="space-y-1 min-w-0 flex-1">
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground truncate">{streamer?.name}</h1>
                        <Badge variant="outline" className={cn("capitalize px-2.5 py-0.5 text-xs font-semibold rounded-full border transition-colors shrink-0", statusBg, statusColor)}>
                    ...
                    <div className="flex flex-wrap items-center text-sm text-muted-foreground gap-2">
                        <span className="capitalize shrink-0">{platform.toLowerCase()}</span>
                        <span className="hidden sm:inline">•</span>
                        <span className="font-mono text-xs opacity-70 hidden sm:inline">ID: {streamer?.id}</span>
