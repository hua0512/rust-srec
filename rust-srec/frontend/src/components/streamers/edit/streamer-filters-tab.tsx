...
export function StreamerFiltersTab({ streamerId, filters, isLoading, onDeleteFilter }: StreamerFiltersTabProps) {
        ...
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-card p-4 rounded-lg border shadow-sm gap-4">
                <div className="space-y-1">
                    <h3 className="text-lg font-medium"><Trans>Recording Filters</Trans></h3>
                    <p className="text-sm text-muted-foreground">
                        <Trans>Define rules to automatically record or skip streams based on title, time, or language.</Trans>
                    </p>
                </div>
                <Button size="sm" onClick={() => { setFilterToEdit(null); setFilterDialogOpen(true); }}>
