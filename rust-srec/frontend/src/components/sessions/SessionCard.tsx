...
import { MoreHorizontal, Play, Film, Clock, HardDrive, Calendar } from 'lucide-react';
        ...
        <Card className="flex flex-col h-full bg-linear-to-b from-card to-card/50 hover:shadow-lg hover:border-primary/20 transition-all duration-300 group">
            <CardHeader className="p-4 pb-2 flex-row gap-3 space-y-0 items-start">
                <Avatar className="h-10 w-10 border border-border shadow-2xs group-hover:border-primary/50 transition-colors">
                    ...
                    <AvatarFallback>{session.streamer_id.substring(0, 2).toUpperCase()}</AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                    ...
                    <h3 className="font-semibold leading-tight truncate mt-1 group-hover:text-primary transition-colors" title={session.title}>
            ...
            <CardContent className="p-4 pt-2 grow">
                ...
                <div className="relative aspect-video bg-muted/50 rounded-md mb-4 overflow-hidden group-hover:ring-1 group-hover:ring-primary/20 transition-all">
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center text-muted-foreground/30 placeholder-icon ${session.thumbnail_url ? 'hidden' : ''}`}>
                    ...
                    <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                        <Button size="sm" variant="secondary" className="w-full gap-2 backdrop-blur-md bg-white/10 hover:bg-white/20 text-white border-0">
            ...
            <CardFooter className="p-2 px-4 border-t bg-muted/20 flex justify-between items-center text-xs text-muted-foreground">
                <Link to="/sessions/$sessionId" params={{ sessionId: session.id }} className="hover:text-primary hover:underline transition-all">
                    View Details
                </Link>
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem>
                            <Link to="/sessions/$sessionId" params={{ sessionId: session.id }} className='flex w-full'>
                                View Details
                            </Link>
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
