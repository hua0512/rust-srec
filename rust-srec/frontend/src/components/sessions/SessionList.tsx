...
export function SessionList({ sessions, isLoading, onRefresh }: SessionListProps) {
    if (isLoading) {
        return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="flex flex-col h-full bg-card/30 rounded-xl border border-border/50 overflow-hidden">
