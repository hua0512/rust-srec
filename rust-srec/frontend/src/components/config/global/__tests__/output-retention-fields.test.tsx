...
describe('output retention settings', () => {
  it('starts disabled and explains that records-only cleanup leaves files behind', () => {
    renderFields();
    expect(
    ...
    expect(
      screen.getByText(/automatic cleanup can no longer delete/),
    ).toBeInTheDocument();
  });

  it('lets the user enable retention and choose either deletion mode', () => {
    ...
    expect(
      screen.getByText(/Permanently deletes tracked output files/),
    ).toBeInTheDocument();
    fireEvent.keyDown(select, { key: 'ArrowDown' });
    fireEvent.click(
