...
import { UpdateTemplateRequestSchema, StreamSelectionConfigObjectSchema } from '../../../../api/schemas';
...
export function StreamSelectionTab({ form }: StreamSelectionTabProps) {
    const rawConfig = form.watch("stream_selection_config");
    ...
    const updateField = <K extends keyof StreamSelectionConfig>(
        ...
        if (Object.keys(newConfig).length === 0) {
            form.setValue("stream_selection_config", null, { shouldDirty: true });
        } else {
            form.setValue("stream_selection_config", JSON.stringify(newConfig), { shouldDirty: true });
        }
    };
