...
export function FilterDialog({ streamerId, open, onOpenChange, filterToEdit }: FilterDialogProps) {
    ...
    useEffect(() => {
        const subscription = form.watch((value, { name }) => {
            if (name === 'filter_type') {
                ...
                let defaultConfig = {};
                switch (type) {
                    case 'TIME_BASED': defaultConfig = { days: [], start_time: '00:00:00', end_time: '23:59:59' }; break;
                    case 'KEYWORD': defaultConfig = { keywords: [], exclude: false, case_sensitive: false }; break;
                    case 'CATEGORY': defaultConfig = { categories: [], exclude: false }; break;
                    case 'CRON': defaultConfig = { expression: '* * * * * *', timezone: 'UTC' }; break;
                    case 'REGEX': defaultConfig = { pattern: '', exclude: false, case_insensitive: false }; break;
                }
    ...
    const onSubmit = (data: z.infer<typeof FormSchema>) => {
        ...
        switch (data.filter_type) {
            case 'TIME_BASED': result = TimeBasedFilterConfigSchema.safeParse(data.config); break;
            case 'KEYWORD': result = KeywordFilterConfigSchema.safeParse(data.config); break;
            case 'CATEGORY': result = CategoryFilterConfigSchema.safeParse(data.config); break;
            case 'CRON': result = CronFilterConfigSchema.safeParse(data.config); break;
            case 'REGEX': result = RegexFilterConfigSchema.safeParse(data.config); break;
        }
        ...
        if (result && !result.success) {
            ...
            error.errors.forEach((err: any) => {
                form.setError(`config.${err.path.join('.')}` as any, { message: err.message });
            });
            return;
        }
    ...
    const renderSubForm = () => {
        switch (filterType) {
            case 'TIME_BASED': return <TimeBasedFilterForm />;
            case 'KEYWORD': return <KeywordFilterForm />;
            case 'CATEGORY': return <CategoryFilterForm />;
            case 'CRON': return <CronFilterForm />;
            case 'REGEX': return <RegexFilterForm />;
            default: return null;
        }
    };
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl gap-0 p-0 overflow-hidden border-none shadow-2xl flex flex-col max-h-[85vh]">
                <DialogHeader className="px-6 py-6 bg-muted/30 border-b flex-shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-1 text-left">
                            <DialogTitle>{isEditing ? <Trans>Edit Filter</Trans> : <Trans>Add New Filter</Trans>}</DialogTitle>
                            <DialogDescription>
                ...
                <div className="flex-1 overflow-y-auto min-h-0">
                    <Form {...form}>
                        <form id="filter-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-6 py-6">
                            <FilterTypeSelector />
                ...
                <DialogFooter className="px-6 py-4 bg-muted/30 border-t flex-shrink-0 gap-2">
                    <Button type="button" variant="outline" onClick={() => onOpenChange(false)}><Trans>Cancel</Trans></Button>
                    <Button type="submit" form="filter-form" disabled={createMutation.isPending || updateMutation.isPending} className="min-w-[120px]">
                        {createMutation.isPending || updateMutation.isPending ? (
                            <span className="animate-pulse"><Trans>Saving...</Trans></span>
                        ) : (
                            isEditing ? <Trans>Save Changes</Trans> : <Trans>Create Filter</Trans>
                        )}
                    </Button>
                </DialogFooter>
            </DialogContent>
