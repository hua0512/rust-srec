...
import { StreamInfoCard, type StreamOption } from '@/components/player/stream-info-card';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
function PlayerPage() {
            ...
            if (response.success && response.media_info) {
                ...
                if (firstStream) {
                    const newPlayer: PlayerInstance = {
                        ...
                        title: variables.url,
                        headers: variables.cookies ? { Cookie: variables.cookies } : undefined,
                        response,
                    };
    ...
    const parseBatchMutation = useMutation({
        ...
        onError: (error) => {
            toast.error(`Batch parse error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        },
    });
    ...
    const isLoading = parseSingleMutation.isPending || parseBatchMutation.isPending;
        ...
        >
            {isImmersive && (
                ...
                >
                    ...
                    <span className="sr-only sm:not-sr-only"><Trans>Exit Immersive</Trans></span>
                </Button>
            )}
                    ...
                    <div className="w-full h-full min-h-[500px]">
                        <PlayerCard
                            ...
                            streamData={
                                ...
                                player.response.media_info?.streams?.find((s: any) =>
                                    (s.url === player.currentStream.url || s.src === player.currentStream.url)
            ...
            <div className="relative z-10 full-width px-6 py-8 pb-32">
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-muted-foreground text-lg max-w-md mx-auto leading-relaxed">
                                <Trans>
                                    Add a stream to start your viewing session.
                                </Trans>
                            </p>
                        </div>
                        <Button
                ...
                <Dialog open={isAddStreamOpen} onOpenChange={setIsAddStreamOpen}>
                    <DialogContent className="sm:max-w-md border-border/50 bg-background/80 backdrop-blur-xl duration-200">
                        <DialogHeader>
                            <DialogTitle><Trans>Add Stream</Trans></DialogTitle>
                            <DialogDescription>
