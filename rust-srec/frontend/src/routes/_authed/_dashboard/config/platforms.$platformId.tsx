...
import { PlatformEditor, EditPlatformFormValues } from '@/components/config/platforms/platform-editor';
import { Skeleton } from "@/components/ui/skeleton";

export const Route = createFileRoute('/_authed/_dashboard/config/platforms/$platformId')({
...
function EditPlatformPage() {
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['config', 'platform', platformId] });
            navigate({ to: '/config/platforms' });
        },
        onError: (error) => toast.error(t`Failed to update platform: ${error.message}`),
    });
    ...
    if (error || !platform) {
        return (
            <div className="flex flex-col items-center justify-center p-20 text-center">
                <h3 className="text-xl font-bold text-destructive"><Trans>Error loading platform</Trans></h3>
                <p className="text-muted-foreground mt-2">{error?.message || t`Platform not found`}</p>
            </div>
        );
    }
