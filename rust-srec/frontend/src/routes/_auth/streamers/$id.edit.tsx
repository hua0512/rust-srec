...
import { streamerApi, sessionApi } from '../../../api/endpoints';
...
function EditStreamerPage() {
  ...
  const { data: sessions, isLoading: isLoadingSessions } = useQuery({
    ...
    queryFn: () => sessionApi.list({ streamer_id: id }),
    enabled: !!id,
  });
  ...
  const { data: filters, isLoading: isFiltersLoading } = useQuery({
    ...
    queryFn: () => streamerApi.getFilters(id),
  });

  const deleteFilterMutation = useMutation({
    mutationFn: (filterId: string) => streamerApi.deleteFilter(id, filterId),
    onSuccess: () => {
