...
export const Route = createFileRoute('/_auth/pipeline/jobs')({
...
function PipelineJobsPage() {
  ...
  const { data: jobs, isLoading: isJobsLoading } = useQuery({
    ...
    queryFn: () => pipelineApi.listJobs(), // TODO: Add filtering support
    refetchInterval: 5000,
  });

  const retryMutation = useMutation({
    mutationFn: pipelineApi.retryJob,
    onSuccess: () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: pipelineApi.cancelJob,
    onSuccess: () => {
