...
- **Filters reuse parsed rules and handle timezone boundaries consistently**
  ...
  Cron and regex definitions use bounded caches. Time-based filters accept explicit IANA timezones and share overnight/DST interval boundaries for matching and wakeups, including overlapping repeated-hour windows. Omitted/null timezones now default to UTC for both rule types. A forward migration and legacy-backup normalization preserve existing TimeBased local schedules; explicit `local` shares the same matching and wake logic. Same-type TimeBased edits that omit timezone retain the stored value, and schema 0.1.8 exports explicit zones. Both filter forms now expose UTC, server-local and IANA timezone choices, preserve the saved zone during edits and display it on filter cards. New filters and clearing the timezone control explicitly select UTC. See [timezone compatibility](../operations/upgrading.md#filter-timezone-compatibility).
...
- **Processors handle short recordings, odd audio codecs and partial upload failures better**

  A thumbnail step no longer fails a whole batch when a recording is shorter than the requested timestamp; it takes the first frame instead, and a recording with no usable frame is passed through. Extracting audio without re-encoding now names the file after the audio it contains (`.mp3`, `.opus`, `.ogg`, `.flac`, `.wav`, and `.mka` for anything else) instead of always `.aac`, which made Opus, Vorbis and MP3 sources fail. Copy and move steps refuse two inputs that would land on the same file name instead of silently keeping only the last one, clean up their temporary copy when a job is cancelled mid-transfer, and detect a cross-device move reliably. When an rclone move or a Baidu Netdisk upload fails part-way, the files that did transfer are recorded as uploaded rather than failed; rclone can now upload files from unrelated folders in one step, and a Baidu Netdisk step rejects two inputs with the same file name before uploading. Execute steps can use placeholders in the scanned output folder, only count files created after the command started as outputs, and the documentation no longer suggests `{output}`, which is empty in workflows.
...
- **Execute templates validate placeholder use before launch**

  Execute rejects unsupported placeholder contexts before starting. Program/args and templates without recognized placeholders retain their behavior. Templates with placeholders support ordinary arguments, redirects and documented shell operators; Windows requires native programs, uses common C/Shell32 argument encoding and checks cmd's size limits. Use program/args or a fixed script with an explicit data interface for unsupported forms; custom argument parsers must accept the supplied encoding. See [Execute](../concepts/pipeline.md#execute-execute) for the supported grammar and migration choices.
