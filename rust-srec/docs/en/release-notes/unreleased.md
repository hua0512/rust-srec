...
- The web container now runs its application server as an ordinary account instead of `root` and sends `X-Content-Type-Options`, `X-Frame-Options`, and `Referrer-Policy` headers on every response; if your reverse proxy already adds these, keep them in one place. Idle live progress and log connections stay open for an hour instead of dropping after a minute. Self-built images no longer include a local database file left in the web source folder.
