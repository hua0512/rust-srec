...
- **Watch and download recordings from the cloud after local files are deleted**

  Rclone presets gain a **Public Access URL** setting so archiving to the cloud no longer breaks session previews. Pick *URL base mapping* for path-preserving storage (S3-compatible buckets such as Qiniu or R2, a CDN, WebDAV/alist): the configured base URL — with the same streamer/title/date placeholders as the destination — is joined with each uploaded file's relative path. Pick *rclone link* for drives like Google Drive or OneDrive: a share link is fetched per file after the upload (optionally with an expiry). Either way the URL is stored with the upload record, and once the local file is gone the session page plays, previews, and downloads straight from the cloud copy, marking such files with a cloud badge. Files produced mid-pipeline and then uploaded (for example a remuxed MP4) now also appear on the session's Recordings tab. A misconfigured base URL fails the upload job before any transfer starts, while a failed share link keeps the job successful and shows the error on the file's upload record instead. The database now enforces one recording row per session file; duplicate rows left behind by older versions are cleaned up automatically on upgrade. See [DAG Pipeline](../concepts/pipeline.md#rclone-public-access-urls-rclone).
