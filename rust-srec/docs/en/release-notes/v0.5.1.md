...
This release removes the **Hooks** tab and its six shell commands — they were stored and merged like any other setting but were never actually run — and notification channels cover what they were meant to do. It also fixes two streamer issues: Bigo rooms are picked up the moment they go live, and switching a streamer off now clears its error state, so switching it back on starts checking straight away.
...
- **Turning a streamer off clears its error history**
  ...
  Switching a streamer off now wipes its recorded error, its consecutive-error count and any retry backoff still counting down. Switching it back on starts checking right away without waiting for the previous backoff to expire, and the card no longer carries the stale error message from before you turned it off.
