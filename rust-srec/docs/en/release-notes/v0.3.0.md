...
This update covers four independent themes: (1) **recording session reliability** — adding a quiet-period for brief network interruptions, cleaning up empty session cards, improving the timeline display, and exposing a per-streamer offline-check override; (2) the **output-root write gate** — fixing a class of failures where rust-srec could not recover from a filesystem issue (disk full, stale Docker bind mount) without a container restart; (3) a **new check-history strip on the streamer details page** that gives you an at-a-glance view of every recent monitor poll, with a tooltip showing exactly which stream quality was picked; (4) **clearer behavior when the concurrent-download limit is hit** — a Queued badge on streamer cards, smarter ordering for high-priority streamers, and no more "everything froze" stalls when the limit saturates. It also adds initial support for backend localization, adds **bandwidth & throughput controls** to the rclone pipeline step so you can throttle uploads from the UI, lets the Mesio CLI run without creating a log file, and includes **platform fixes** for Huya (rectification notices, CDN priority mapping) and RedBook (offline parsing).
...
- **Quiet period for brief disconnects: short interruptions no longer end the session**

  When the upstream CDN rotates, the network is briefly interrupted, or the streamer briefly reconnects, the active download ends cleanly. Previously every disconnect immediately ended the session — and the next LIVE detection would create a fresh session card, so repeated interruptions would create multiple zero-byte cards on the dashboard.

  Now the disconnect enters a short waiting window (default tracks the "offline detection" config, around one minute). If LIVE is detected again within the window, **recording continues** under the same session — no new card appears. If no LIVE is observed by the window's end, the session ends.
...
- **Per-streamer offline-check override**

  The streamer, template, and platform editors now have an **Offline Check** card where you can override how quickly the scheduler decides a stream has ended — for a single streamer, or for everyone using a template/platform. Leave a field blank to inherit the global setting. Useful when one streamer needs a longer quiet period than the rest, or when you want a particular account confirmed offline faster.
...
- **Graceful shutdown won't start new recordings mid-shutdown**
  ...
  Previously, queued recordings could acquire a newly released slot and start while the system was shutting down. Queued recordings are now rejected during shutdown.
...
- **Huya: rectification notices no longer get streamers stuck offline** ([#557](https://github.com/hua0512/rust-srec/pull/557))

  When Huya shows the temporary "该主播涉嫌违规，正在整改中" review notice, rust-srec used to treat it as a permanent ban and stop polling — so recordings stayed dark long after the streamer was back. The notice is now treated as a normal offline state and polling continues on the regular cadence.

- **Huya: better CDN selection for some rooms** ([#513](https://github.com/hua0512/rust-srec/pull/513) / [#514](https://github.com/hua0512/rust-srec/pull/514))

  Recordings sometimes ended up on the CDN Huya itself rated worst, and a few rooms had a perfectly usable CDN dropped from the list entirely. Selection now matches what Huya's own web player picks, and your **Preferred / Blacklisted CDNs** settings are honored properly.

- **RedBook: ended streams now show as offline cleanly** ([#510](https://github.com/hua0512/rust-srec/pull/510))

  When a RedBook stream ended, the monitor logged a hard error every poll instead of marking the streamer offline, so the dashboard kept showing it as "recording". Ended streams now transition to offline normally.

## Frontend

- **"Account Not Found" badge for deleted streamers** ([#519](https://github.com/hua0512/rust-srec/pull/519))

  Streamers whose accounts no longer exist on the platform (deleted, banned, mistyped URL) used to share the same red **Monitoring Stopped** badge as disk-full and other generic errors. They now get a dedicated orange **Account Not Found** badge with a tooltip explaining the likely causes, so you can tell at a glance which streamers actually need attention.

- **Brand icons restored for Twitch and YouTube** ([#545](https://github.com/hua0512/rust-srec/pull/545))

  An icon-library update upstream removed the Twitch and YouTube logos. Both are back in the platform icon map, so the dashboard looks the same as before.
...
- Added **output-root write gate** for recording filesystem failure resilience ([#508](https://github.com/hua0512/rust-srec/issues/508))
  ...
  When the recording disk fills or the target mount becomes unwritable, rust-srec now pauses recordings at the filesystem boundary, exposes the situation in `/api/health`, emits one critical notification with actionable recovery text, and auto-recovers when the filesystem becomes writable again — without restart for the common out-of-space case. For the specific case where a Docker bind mount has been broken by host-side cleanup (e.g., BaoTa panel's move-to-trash on a mounted directory), the gate cannot auto-recover (it's a Linux VFS limitation unrelated to rust-srec), but it now detects the situation within one monitor tick, stops repeated retries and their log messages, and tells the user to restart the container with clear recovery instructions.

  **Replaces the 40+ repeated failure reports in #508** with a single `Degraded` status and one notification. See the new [Docker troubleshooting guide](../getting-started/docker.md#storage-cleanup) for the safe cleanup paths that avoid stale mounts.
...
- Added **`StreamerState::OutOfSpace` runtime wiring**
  ...
  The state existed in the domain model but was never set at runtime. It is now set when the write gate blocks a streamer, and clears automatically when the gate recovers. Visible in the streamer list as a stop-state badge.
...
- Added **one-shot startup probe** for configured output roots
  ...
  On container boot, after streamer hydration and before the scheduler starts, the gate runs a bounded 5-second probe per configured root (via `spawn_blocking` + timeout) to detect broken mounts at startup instead of waiting for the first monitor tick to try starting a download.
...
- **Rclone processor switched to a typed `RcloneConfig` struct.** The processor previously read individual fields from a generic `serde_json::Value`; it now uses a `#[derive(Deserialize)]` struct like every other processor in the crate. Behavior is unchanged. The struct replaces repeated `.get(…).and_then(…)` calls and explicitly defines the new throughput fields.
...
- `GET /sessions` default behavior changed: zero-byte ended sessions are no longer returned (their empty cards no longer appear on the dashboard by default). Pass `?include_empty=true` to see all records. `GET /sessions/:id` is unaffected.
