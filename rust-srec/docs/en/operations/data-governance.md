...
Deletion takes effect immediately but is not always instantaneous. The streamer disappears from the interface and the API at once, and nothing starts new work for it; if it was recording, that recording is closed out first, and if post-processing is still running the streamer's own row is removed once that finishes — including across a restart. Until then the streamer's URL is still taken, so re-adding the same address, importing a configuration that still contains it, or deleting a template it uses is refused with "still being removed; try again shortly". Scripts that delete a streamer and then re-create it at the same URL should retry.
