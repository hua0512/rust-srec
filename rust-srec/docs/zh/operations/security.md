...
- `AUTH_DISABLED=true` 仅用于本机开发。后端只会在绑定回环地址时接受它，生产流程不得依赖该选项。该模式下浏览器跨域请求被限制在 `API_CORS_ORIGINS` 允许列表内，因此同一浏览器中打开的其他网页无法驱动未认证的 API；仅在你能控制的来源上放宽该列表。
- 连续登录失败会按用户名和来源地址分别限流，被限流的请求直接返回 `429` 和 `Retry-After` 等待时间，不再执行一次密码哈希。
