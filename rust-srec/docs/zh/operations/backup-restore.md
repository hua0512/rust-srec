...
过滤器和通知订阅按每批最多 500 个所属实体查询，保持主播/渠道顺序和各实体原有的
子项顺序。批量读取失败时，后端会逐个重试该批实体，使其他正常实体的数据仍能
保留在导出中。与此前相同，关联数据查询仍然失败的实体会带有空的过滤器/订阅列表；
后端会记录恢复和失败数量。主要配置列表读取失败时，导出仍会中止。
...
对于结构版本为 `0.1.3` 或更高且用户列表非空的导出文件，用户按用户名匹配。
导入会在任何配置写入之前，根据最终用户集合检查邮箱唯一性，包括 `merge`
模式保留的账号。邮箱比较保留大小写和空白：`User@example.com` 与
`user@example.com` 被视为不同值。空字符串也必须唯一；多个用户可以不设置邮箱（`null`）。

导入可以交换两个待更新用户的邮箱，也可以把某个待更新用户释放的邮箱分配给另一个用户。
`replace` 模式还可以复用被省略用户的邮箱。用户名匹配的现有用户会保留 ID 和创建时间。
如果后续任意导入写入失败，邮箱改动会与其他配置写入一起回滚。
省略用户列表、用户列表为空或结构版本早于 `0.1.3` 时，两种模式都会保留现有用户。

每次成功导入都会撤销全部刷新令牌及其绑定的访问令牌会话，包括省略用户的
`merge` 导入。导入完成后请重新登录。
被拒绝的导入不会撤销令牌。
