...
本次更新围绕两个独立主题：(1) **录制会话稳定性**——修复短暂断流场景下录制悄无声息中止、空会话卡片堆积、会话时间线显示错乱等问题；(2) **输出根写入门（output-root write gate）**——修复了一类"rust-srec 遇到文件系统问题（磁盘满、Docker 绑定挂载失效）必须重启容器才能恢复"的故障。同时引入了后端本地化的基础设施。

## 录制会话稳定性

围绕"短暂断流不应该停止录制、空会话不应该出现在仪表盘"的目标，本次修复了多条相互关联的问题。

- **修复短暂断开后录制悄无声息地中止**

  当上游 CDN 切换、网络抖动或主播端简短重连时，下载会干净地结束。此前在这种情况下，会话状态在系统内显示仍然"在线"，但实际下载进程已经停止——意味着主播继续在直播，rust-srec 却只录到了断开前的一段。最长可观察到的丢失录制时长达 1 个多小时。

  现在断开会进入一个简短的等待期（默认与"离线检测"配置相同，约一分钟）。如果在窗口内重新检测到直播，下载会自动重启并复用同一会话——录制无缝继续。如果窗口内没有重新出现直播，会话才会被认定为结束。

- **不再把瞬时 HTTP 404 当作主播下线**

  Douyu 等平台在直播刚刚恢复时，新签发的流地址需要几秒才能在 CDN 边缘节点同步——这段时间对该地址的请求会返回 404。HLS 流也存在类似的瞬时 404 场景（滑动窗口剔除、签名 URL 过期、节点失同步）。

  此前 rust-srec 把任意 404 视作"主播确实下线"的权威信号，立即结束会话。即便平台监控仍然报告 LIVE，新一轮检测又会创建一个空会话——往往一次直播开始时连续产生 3 张零字节卡片。

  现在 404 不再单独作为下线判定的依据。真正的下线信号通过两条更精确的渠道：连续多次网络失败（数量与窗口由"离线检测"配置决定，与 hysteresis 共用同一组参数），或 HLS 的 `#EXT-X-ENDLIST` 标签（平台明确告知流已结束）。

- **HLS 直播正常结束时立即关闭会话**

  当 HLS 流播放列表带有 `#EXT-X-ENDLIST` 标签（平台明确表示直播已结束）时，会话现在会**立即**权威结束。此前需要等约 90 秒的等待期超时——延迟了后处理流水线（合并、上传等）的开始。

- **零字节"幽灵会话"清理**

  小于 `min_segment_size_bytes` 阈值的录制片段会被自动丢弃（避免保留无用的几秒钟记录），但此前对应的会话行仍会留在数据库里，在仪表盘上显示为零字节卡片。这次补齐两层清理：

  - **API 默认过滤**——会话列表 API 默认隐藏零字节的已结束会话。活跃会话（尚未结束）始终保留。需要诊断时可通过 `?include_empty=true` 调出来，或直接通过会话 ID 访问已知会话。
  - **后台定期清理**——空会话行在结束 5 分钟后会被自动从数据库删除（默认扫描间隔 30 分钟）。相关的弹幕统计、分段记录、生命周期事件等子行通过外键 `ON DELETE CASCADE` 一并清理。
