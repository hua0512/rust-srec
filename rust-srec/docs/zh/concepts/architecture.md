...
[停止 Streamlink 录制](engines.md#停止-streamlink-录制)。
...
同时移除了未使用的 `StreamerManager::record_error` 和 `StreamerRepository::record_streamer_error`。监控保留事务式错误写入与 `disabled_until_for_error_count` 计算。退避保留配置的阈值，从 60 秒开始翻倍，最多一小时；即使存储的错误计数很大，也会安全达到上限，不会溢出。数据库与缓存发布现共享上述已提交写入边界。
