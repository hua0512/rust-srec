...
工作流步骤不会收到输出路径，因此 `{output}`、`{outputN}` 和 `{outputs_json}` 在工作流中为空。请在参数里指定文件名，并设置 `scan_output_dir`（支持相同的占位符），新文件就会传递给下一步：命令开始前目录中不存在、且在命令开始后修改过的文件即为该步骤的输出。未设置扫描目录时，输入会原样传递。
...
- **重试**：BaiduPCS-Go 的退出码无法反映上传结果，rust-srec 会解析其逐文件输出来判定成败。无论是运行内重试还是手动重试任务，都只会重新上传尚未确认结果的文件；配合默认的 `skip` 策略和秒传检测，部分失败后的重试开销很小。批量上传中部分文件失败时，步骤会失败，并按文件分别记录结果（已上传、已跳过或失败）。由于所有文件都直接放在目标文件夹下，文件名相同的两个输入会在上传前被拒绝。
