...
媒体处理器共享命令错误选择和批量结果汇总逻辑。输出、跳过及成功输入和日志仍按
输入顺序汇总；各处理器继续负责输出命名、发布、回滚和源文件删除。传输处理器在
上传可能删除源文件之前记录文件大小，只有确认源文件不存在时才视为此前已处理。
I/O 错误会使该输入保持待处理。这些共享辅助逻辑不统一 remux 与 ASS 的路径身份
判断规则，也不改变各处理器专用的跳过元数据。
