...
后端 `TIME_BASED` 过滤器 JSON 支持可选的 IANA `timezone`，例如
`"Europe/Madrid"`。匹配、下一次开始和结束统一使用该时区。跨夜区间归属于开始
时的星期；时钟重复的小时内，开始使用较早时刻，结束使用较晚时刻，重叠或相接的
区间保持连续匹配。不存在的本地边界会向后移动到三小时内首个有效分钟；跳过时间
更长的日期窗口会被省略。

省略时区保留兼容行为：时间规则使用服务器本地时区，cron 默认 UTC。比较两种规则
时应显式指定时区；前端时间规则表单尚未提供此字段。Cron 按分钟匹配，包括含秒的
