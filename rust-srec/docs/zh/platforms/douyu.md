...
App 机型、生成的 User-Agent 和设备 ID 在首次请求 App 播放时初始化，之后在重试及切换 CDN 时复用。离线检查和 Web 提取不会初始化 App 设备。设备选项仍在配置提取器时进行校验。服务端注册需要联网，超时为 5 秒，同一提取器会复用结果；注册失败会报错，不会静默切换身份。如需跨提取器实例或程序重启保持身份，请配置 `device_id` 或有效的 `acf_did` Cookie。
