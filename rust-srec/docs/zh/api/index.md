...
来源地址取自 TCP 连接的对端，且不信任 `X-Forwarded-For` 与 `X-Real-IP`，因此**在本项目自带的前端容器、nginx 或任何反向代理之后，所有登录都会被归到代理的地址上**——按地址的配额是开销上限，而不是针对某个客户端的锁定。超过 128 个字符的用户名会在两个配额生效之前直接返回 `400`。两个配额均可配置，详见[配置](../getting-started/configuration.md#登录限流)。
