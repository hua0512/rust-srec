...
impl MaintenanceScheduler {
    ...
    async fn get_freeable_space(&self) -> Result<i64, crate::Error> {
        let result: (i64,) = sqlx::query_as("SELECT freelist_count * page_size FROM pragma_freelist_count(), pragma_page_size()")
            .fetch_one(&self.pool)
            .await
    ...
    pub async fn cleanup_dead_letters(&self) -> Result<i32, crate::Error> {
        let cutoff = Utc::now() - chrono::Duration::days(self.config.dead_letter_retention_days as i64);
        let cutoff_str = cutoff.format("%Y-%m-%d %H:%M:%S").to_string();
        
        let result = sqlx::query(
            "DELETE FROM notification_dead_letter WHERE created_at < ?"
        )
            .bind(&cutoff_str)
            .execute(&self.pool)
            .await
