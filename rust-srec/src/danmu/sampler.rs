...
impl DanmuSamplingConfig {
    ...
    pub fn velocity(min_interval_secs: u64, max_interval_secs: u64, target_danmus_per_sample: u32) -> Self {
        Self::Velocity {
            min_interval_secs,
            max_interval_secs,
    ...
    fn should_sample(&self, now: DateTime<Utc>) -> bool {
        match self.last_sample {
            ...
            Some(last) => {
                ...
                elapsed >= chrono::Duration::from_std(self.interval).unwrap_or(chrono::Duration::MAX)
            }
        }
    }
...
impl VelocitySampler {
    ...
    fn calculate_velocity(&self, now: DateTime<Utc>) -> f64 {
        let cutoff = now - chrono::Duration::from_std(self.window_duration).unwrap_or(chrono::Duration::seconds(60));
        
        let total_messages: u32 = self.velocity_window
            .iter()
...
impl DanmuSampler for VelocitySampler {
    ...
    fn should_sample(&self, now: DateTime<Utc>) -> bool {
        match self.last_sample {
            ...
            Some(last) => {
                let elapsed = now.signed_duration_since(last);
                elapsed >= chrono::Duration::from_std(self.current_interval).unwrap_or(chrono::Duration::MAX)
            }
        }
    }
    ...
    fn reset(&mut self) {
        ...
        self.current_interval = Duration::from_secs(
            (self.min_interval.as_secs() + self.max_interval.as_secs()) / 2
        );
    }
}
...
mod tests {
    ...
    fn test_velocity_sampler_adjusts_interval() {
        ...
        assert!(interval <= Duration::from_secs(15), "Interval should decrease with high velocity");
    }
    ...
    fn test_velocity_sampler_low_velocity() {
        ...
        let interval = sampler.current_interval();
        assert_eq!(interval, Duration::from_secs(30), "Interval should be max with low velocity");
    }
