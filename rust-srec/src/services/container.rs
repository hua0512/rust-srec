...
/// Default upper bound for how long a queued download attempt may use
/// the URLs captured at the original `StreamerLive` event before the
/// pipeline refetches the live state from the monitor. URLs on signed
/// CDN endpoints (Douyin, Huya, etc.) can expire on the order of
/// minutes; below this threshold we trust the captured URLs to avoid
/// hammering the platform on every brief queue wait.
const DEFAULT_QUEUE_FRESHNESS_MS: i64 = 60_000;

/// Read the freshness threshold from the environment (override) or
/// fall back to [`DEFAULT_QUEUE_FRESHNESS_MS`]. Centralised here so
/// both `run_live_download_pipeline` and tests share one value.
fn queue_freshness_threshold_ms() -> i64 {
    std::env::var("RUST_SREC_QUEUE_FRESHNESS_MS")
        .ok()
        .and_then(|s| s.parse::<i64>().ok())
        .filter(|&v| v >= 0)
        .unwrap_or(DEFAULT_QUEUE_FRESHNESS_MS)
}
...
async fn run_live_download_pipeline(
    ...
    if waited_ms > queue_freshness_threshold_ms() {
        debug!(
            streamer_id = %streamer_id,
            waited_ms,
