...
//! - `parking_lot::RwLock<Arc<GpuSnapshot>>` for the rich snapshot. Reads
//!   on the `/api/health` hot path are an `Arc::clone` plus a cheap
//!   struct copy of the pre-built [`ComponentHealth`] — no formatting and
        ...
        match probe {
            Ok(Ok(out)) if out.status.success() => {
                ...
                Some(Arc::new(Self {
                    ...
                    snapshot: RwLock::new(Arc::new(GpuSnapshot::unknown())),
