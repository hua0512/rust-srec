...
-- jobs left PROCESSING by an unclean stop) must not leave the previous attempt's
-- percentage readable until the next flush from upsert_job_execution_progress.
-- PROCESSING is the only entry into PENDING this trigger has to clean: every
-- other source status is terminal, and trg_job_terminal_clears_progress already
-- deleted the row on the way in.
--
